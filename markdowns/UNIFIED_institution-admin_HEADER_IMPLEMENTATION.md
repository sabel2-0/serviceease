...
- Returns coordinator information including:
  - Institution Admin name (first_name, last_name)
...
- `institutions` table has a `user_id` column that points to The institution_admin who owns that institution
...
- Institution Admin name from `user.firstName` and `user.lastName`
...
3. Verify:
   ...
   - Blue banner displays The institution_admin's full name
