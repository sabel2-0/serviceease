...
app.post('/api/institutions', authenticateAdmin, async (req, res) => {
    try {
        ...
        // Generate a unique institution_id
        // Format: INST-YYYYMMDD-XXXXXX (e.g., INST-20251027-ABC123)
        const generateInstitutionId = () => {
            const date = new Date();
            const dateStr = date.toISOString().slice(0, 10).replace(/-/g, '');
            const randomStr = Math.random().toString(36).substring(2, 8).toUpperCase();
            return `INST-${dateStr}-${randomStr}`;
        };
        
        let institution_id = generateInstitutionId();
        let attempts = 0;
        const maxAttempts = 5;
        ...
        while (attempts < maxAttempts) {
            ...
            institution_id = generateInstitutionId();
            attempts++;
        }
        
        if (attempts >= maxAttempts) {
            return res.status(500).json({ error: 'Failed to generate unique institution ID' });
        }
