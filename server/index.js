...
app.post('/api/institutions', authenticateAdmin, async (req, res) => {
    try {
        ...
        // Get the next ID to generate institution_id
        const [maxIdResult] = await db.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM institutions');
        const nextId = maxIdResult[0].next_id;
        const generatedInstitutionId = `INST-${String(nextId).padStart(3, '0')}`;
        
        console.log('Generated institution_id:', generatedInstitutionId);
        console.log('About to execute INSERT query...');
        
        // Insert with the generated institution_id (avoiding trigger conflict)
        const [result] = await db.query(
            'INSERT INTO institutions (institution_id, name, type, address) VALUES (?, ?, ?, ?)',
            [generatedInstitutionId, name, type, address]
        );
