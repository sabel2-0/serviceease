...
// Ensure institutions table exists with correct schema
async function ensureInstitutionsTable() {
    try {
        // First check if institutions table exists and has the correct structure
        const [tables] = await db.query("SHOW TABLES LIKE 'institutions'");
        ...
        if (tables.length === 0) {
            ...
            `);
            console.log('Created institutions table with correct schema');
        } else {
            // Table exists, check if it has id column
            const [columns] = await db.query("SHOW COLUMNS FROM institutions");
            const columnNames = columns.map(col => col.Field);
            
            if (!columnNames.includes('id')) {
                // Need to recreate table with correct schema
                // First backup existing data
                await db.query(`CREATE TABLE institutions_backup AS SELECT * FROM institutions`);
                
                // Drop and recreate
                await db.query(`DROP TABLE institutions`);
                await db.query(`
                    CREATE TABLE institutions (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        institution_id VARCHAR(50) UNIQUE,
                ...
                try {
                    await db.query(`
                        INSERT INTO institutions (institution_id, name, type, address, created_at, updated_at)
                        SELECT institution_id, name, type, address, created_at, updated_at 
                        FROM institutions_backup
                    `);
                    await db.query(`DROP TABLE institutions_backup`);
                    console.log('Migrated institutions table to new schema');
                } catch (err) {
        ...
        // Ensure trigger exists to auto-generate institution_id
        await db.query(`DROP TRIGGER IF EXISTS after_institution_insert`);
        await db.query(`
            CREATE TRIGGER after_institution_insert 
            AFTER INSERT ON institutions
            FOR EACH ROW
            BEGIN
                IF NEW.institution_id IS NULL OR NEW.institution_id = '' THEN
                    UPDATE institutions 
                    SET institution_id = CONCAT('INST-', LPAD(NEW.id, 6, '0'))
                    WHERE id = NEW.id;
                END IF;
            END
        `);
        
        console.log('Ensured institutions table and triggers exist');
    } catch (err) {
        console.error('Failed to ensure institutions table:', err.message);
    }
}

// Initialize database tables that must exist
ensureInstitutionsTable();
ensurePrintersTable();
...
app.post('/api/institutions', authenticateAdmin, async (req, res) => {
    try {
        ...
        // Insert without institution_id first (let auto-increment id handle it)
        const [result] = await db.query(
            'INSERT INTO institutions (name, type, address) VALUES (?, ?, ?)',
            [name, type, address]
        );
        
        // Generate institution_id based on the auto-increment id
        const generatedInstitutionId = `INST-${String(result.insertId).padStart(6, '0')}`;
        
        console.log('Generated institution_id:', generatedInstitutionId, 'for id:', result.insertId);
        
        // Update the row with the generated institution_id
        await db.query(
            'UPDATE institutions SET institution_id = ? WHERE id = ?',
            [generatedInstitutionId, result.insertId]
        );
