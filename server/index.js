...
// Ensure institutions table exists and drop conflicting trigger
async function ensureInstitutionsTable() {
    try {
        // Drop the trigger if it exists to avoid conflicts
        await db.query('DROP TRIGGER IF EXISTS generate_institution_id').catch(() => {});
        console.log('Dropped generate_institution_id trigger (if existed)');
        ...
        `);
        console.log('Ensured institutions table exists without trigger');
    } catch (err) {
        console.error('Failed to ensure institutions table exists:', err);
    }
}
