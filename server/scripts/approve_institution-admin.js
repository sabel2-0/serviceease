...
async function approveinstitution_admin(email, institutionId) {
...
(async () => {
    try {
        ...
        if (action === 'approve' && email && institutionId) {
            await approveinstitution_admin(email, institutionId);
            process.exit(0);
        }
        console.log('Usage:');
        console.log('  node server/scripts/approve_institution_admin.js approve user@example.com INST-004');
        process.exit(1);
    } catch (err) {
