...
function verifyRole(allowedRoles) {
    ...
    if (!allowedRoles.includes(user.role)) {
        ...
        if (currentPath !== targetPath) window.location.href = targetPath;
        return false;
    }
    return true;
}
...
function getCurrentPathInfo() {
    ...
    if (pathParts.length >= 2 && pathParts[0] === 'pages') return {
        section: pathParts[1],
...
function verifySectionAccess() {
    ...
    if (user.role === 'operations_officer' && pathInfo.section === 'admin' && sharedAdminPages.includes(pathInfo.page)) return true; // Allow access to shared pages
    ...
    if (allowedSection && pathInfo.section !== allowedSection) {
        ...
        // Only redirect if not already on the correct dashboard
        if (window.location.pathname !== targetPath) window.location.href = targetPath;
        return false;
    }
    return true;
}
...
function showEmailVerificationAlert() {
    ...
    setTimeout(()=>{
        if (alertContainer.parentElement) alertContainer.remove();
    }, 10000);
}
...
async function sendVerificationEmail() {
    ...
    try {
        ...
        if (response.ok) alert('Verification email sent! Please check your inbox.');
        else throw new Error(data.error || 'Failed to send verification email');
    } catch (error) {
...
async function loginUser({ email, password }) {
    try {
        ...
        if (data.user.approvalStatus === 'pending') localStorage.setItem('showApprovalAlert', 'true');
        // Get the base path dynamically
        const currentPath = window.location.pathname;
        let basePath = '';
        // If we're in the pages directory
        if (currentPath.includes('/pages/')) // We're in a pages subdirectory
        {
            if (currentPath.split('/').filter(Boolean).length > 2) basePath = '../'; // Go up one level (e.g., from /pages/subdir/ to /pages/)
        } else // We're somewhere else, default to going to pages/
        basePath = 'pages/';
        // Redirect based on role
        if (data.user.role === 'admin') window.location.href = `${basePath}admin/admin.html`;
        else if (data.user.role === 'operations_officer') window.location.href = `${basePath}operations-officer/operations-officer.html`;
        else if (data.user.role === 'technician') window.location.href = `${basePath}technician/technician.html`;
        else if (data.user.role === 'requester') // Requester users have a dedicated mobile/compact UI
        window.location.href = `${basePath}requester/requester.html`;
        else // All other users (including coordinators) go to coordinator.html
...
async function registerUser(formData) {
    try {
        ...
        if (formData.institutionId) registrationData.append('institutionId', formData.institutionId);
        ...
        if (frontIdInput && frontIdInput.files[0]) registrationData.append('frontId', frontIdInput.files[0]);
        if (backIdInput && backIdInput.files[0]) registrationData.append('backId', backIdInput.files[0]);
        if (selfieInput && selfieInput.files[0]) registrationData.append('selfie', selfieInput.files[0]);
...
//# sourceMappingURL=register.ad25af97.js.map
