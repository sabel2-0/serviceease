/**
 * institutionAdmin Header Loader
 * Automatically loads and initializes the unified header component for institutionAdmin pages
 */
...
(function() {
    ...
    /**
     * Load institutionAdmin profile data from API
     */
    async function loadinstitutionAdminProfile() {
        try {
            ...
            console.log('Fetching institutionAdmin profile from /api/institutionAdmin/profile');
            const response = await fetch('/api/institutionAdmin/profile', {
                headers: {
            ...
            if (!response.ok) {
                console.error('Failed to fetch institutionAdmin profile:', response.status, response.statusText);
                throw new Error('Failed to fetch institutionAdmin profile');
            }

            const data = await response.json();
            console.log('institutionAdmin profile data:', data);
            ...
            // Update institutionAdmin name
            const institutionAdminNameEl = document.getElementById('header-institution_admin-name');
            if (institutionAdminNameEl && data.first_name && data.last_name) {
                institutionAdminNameEl.textContent = `${data.first_name} ${data.last_name}`;
            }
            
        } catch (error) {
            console.error('Error loading institutionAdmin profile:', error);
            useFallbackData();
        }
    }
    ...
    function useFallbackData() {
        ...
        if (user.firstName && user.lastName) {
            const institutionAdminNameEl = document.getElementById('header-institution_admin-name');
            if (institutionAdminNameEl) institutionAdminNameEl.textContent = `${user.firstName} ${user.lastName}`;
        }
    }
     ...
     */
    async function loadinstitutionAdminHeader(options = {}) {
        try {
            // Determine the correct path to the components directory
            const paths = [
                '../../components/institution_admin-header.html',
                '../components/institution_admin-header.html',
                '/components/institution_admin-header.html',
                '/client/src/components/institution_admin-header.html'
            ];
            ...
            if (!html) {
                console.error('Could not load institutionAdmin header from any path');
                return;
            }
            
            // Insert the header into the container
            const container = document.getElementById('institution_admin-header-container');
            if (container) {
            ...
            console.error('Error loading institutionAdmin header:', error);
        }
    }
    
    // Make the function globally available
    window.loadinstitutionAdminHeader = loadinstitutionAdminHeader;
